...
public class MouseListener {
    ...
    public static Vector2f getScreen() {
        ...
        float currentY = (getY() - get().gameViewportPos.y);
        currentY = (1.0f - (currentY / get().gameViewportSize.y)) * 1200.0f;
    ...
    // Get the scale-adjusted mouse position in world coordinates
    public static Vector2f getScaledWorld() {
        // Get current DPI scale
        float[] xScale = new float[1];
        float[] yScale = new float[1];
        org.lwjgl.glfw.GLFW.glfwGetWindowContentScale(Window.getWindow(), xScale, yScale);
        float scale = Math.max(xScale[0], yScale[0]);
        
        // Apply scale to coordinates
        float scaledX = getX() / scale;
        float scaledY = getY() / scale;
        
        float currentX = scaledX - get().gameViewportPos.x;
        currentX = (2.0f * (currentX / get().gameViewportSize.x)) - 1.0f;
        float currentY = (scaledY - get().gameViewportPos.y);
        currentY = (2.0f * (1.0f - (currentY / get().gameViewportSize.y))) - 1;
